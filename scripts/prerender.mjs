// scripts/prerender.mjs  (lapland-blog)
//
...
//
// Dynamic post + category slugs are discovered from src/data at run time so the
// route list never drifts from the app. Auth/transactional routes (signin,
// unsubscribe, /me, /admin) and the DB-backed /by/:handle profiles are
// intentionally NOT prerendered (no static SEO value / no enumerable slugs).
...
// ---- Static public routes (must match the routes built in src/App.tsx) -----
const STATIC_ROUTES = [
  '',
  '/start-here',
  '/top-reads',
...
async function main() {
  ...
  const postSlugs = await discoverSlugsFrom(join('src', 'data', 'posts.ts'))
  const catSlugs = await discoverSlugsFrom(join('src', 'data', 'categories.ts'))
  const ROUTES = [
    ...STATIC_ROUTES,
    ...catSlugs.map((s) => `/category/${s}`),
    ...postSlugs.map((s) => `/post/${s}`),
  ]
  console.log(
    `[prerender] routes: ${STATIC_ROUTES.length} static + ${catSlugs.length} categories + ` +
      `${postSlugs.length} posts = ${ROUTES.length} × ${LOCALE_PREFIXES.length} locales = ` +
  ...
  for (const prefix of LOCALE_PREFIXES) {
    for (const route of ROUTES) {
      ...
      try {
        ...
        // lapland-blog has no <footer> on every route; gate on body + canonical.
